...
class HyperlprRun(tornado.web.RequestHandler):
    '''
    使用 hyperlp 的 run 方法
    '''
    ...
    def post(self):
        ...
        # 进行
        image_tmp = img.copy()
        image = cv2.cvtColor(numpy.asarray(image_tmp), cv2.COLOR_RGB2BGR)
        ...
        if is_draw != '0':
            ...
            for i, r in enumerate(res):
                ...
                points = numpy.array([[left,top],[right,top],[left,bottom],[right,bottom]])
                rectarea = cv2.minAreaRect(points)
                box = cv2.boxPoints(rectarea)
